...
class NetworkOperations(private val mainActivity: MainActivity) {
    companion object {
        ...
        lateinit var defaultOperator: NetworkOperations
    }
     ...
     */
    fun send(file: File, textFile: File, detailActivity: DetailActivity? = null) {
        textFile.writeText(MainAdapter.STATUS_AWAITING.toString())
        awaitingFiles.add(textFile.name)

        updateUi(detailActivity)
     ...
     */
    fun saveAwaitingFiles() {
        File(mainActivity.filesDir, "awaitingFiles").apply {
            delete()
